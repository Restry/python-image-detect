...
class BWImageCompare(object):
    ...
    def mse(self):
        ...
        if not hasattr(self, '_mse'):
            tmp = sum((a-b)**2 for a, b in list(zip(self.imga_int, self.imgb_int)))
...
if __name__ == '__main__':
    ...
    print('Comparing %d images:' % len(images))

    results, i = {}, 1
    for namea, imga in list(images.items()):
        for nameb, imgb in list(images.items()):
            if namea == nameb or (nameb, namea) in results:
                continue

            print(' * %2d / %2d:' % (i, tot), end=' ')
            print(namea, nameb, '...', end=' ')
